...
- **Backdrop in item detail card** — movies, TV shows, games, and manga display backdrop image as background in the detail card. Movies/TV use TMDB backdrop, games use IGDB artwork (`artwork_url` in `games`), manga uses AniList banner (`banner_url` in `manga_cache`). DB migration v32. All backdrop URLs persisted to DB, included in export/import. Full-bleed backdrop with vertical gradient fade (matching search sheet style), content wrapped in frosted-glass container (`media_detail_view.dart`)
- **Detailed API error info with copy button** — all 7 API clients (IGDB, TMDB, SteamGridDB, AniList, VNDB, Steam, RetroAchievements) now capture full debug info on errors: API name, request URL+method, HTTP status, DioException type, underlying cause, and response body excerpt. Error display shows user-friendly message with "Copy error details" button for debugging. New files: `api_error_detail.dart` (detail builder), `api_error_extract.dart` (pattern-matching extractor), `api_error_display.dart` (UI widget). 2 localization keys EN+RU
- **API connection timeouts** — all 7 API clients now have 5-second `connectTimeout` and `receiveTimeout` (was unlimited/infinite). Prevents UI from hanging indefinitely on network issues
