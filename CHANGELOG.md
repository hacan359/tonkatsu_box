...
### Added
- **Steam Library import** — new `SteamApi` client (`steam_api.dart`) fetches user's owned games via Steam Web API. `SteamImportService` (`steam_import_service.dart`) orchestrates the full import pipeline: fetch library → filter DLC/soundtracks/demos → match each game to IGDB → add found games to a new "Steam Library" collection (PC platform, status based on playtime) → add unfound games to wishlist with media type hint. Playtime saved as user comment (`Steam: 2.1h`), last played date as `startedAt`. Rate limiting (4 req/sec) for IGDB. Progress callback with stage/current/total/stats
- **Steam import UI** — `SteamImportScreen` + `SteamImportContent` with 3 states: input (API key + Steam ID fields with clickable helper links to steamcommunity.com and steamidfinder.com), progress (linear indicator + live stats), result (imported/wishlisted/skipped counts + "Open Steam Library" button navigating to the created collection). IGDB connection warning when not configured. Accessible from Settings > Import section (`steam_import_screen.dart`, `steam_import_content.dart`, `settings_screen.dart`)
...
- **Steam import localization** — 24 new keys in EN and RU ARB files for all Steam import UI strings (`app_en.arb`, `app_ru.arb`)
- **Steam test infrastructure** — `MockSteamApi`, `MockSteamImportService` in `mocks.dart`, `createTestSteamOwnedGame` builder in `builders.dart`. 25 tests for `SteamApi` (parsing, errors, shouldSkip), 17 tests for `SteamImportService` (import flow, statuses, wishlist, duplicates, progress, exact match)
