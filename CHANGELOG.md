...
- **Search detail sheets redesign** — all 4 detail bottom sheets (Game, Movie/TV, VN, Manga) redesigned: `SourceBadge` with external link (IGDB/TMDB/VNDB/AniList), year inline with title, compact genre chips (matching item detail card style), backdrop image behind header (IGDB artworks for games, TMDB backdrop for movies/TV, AniList banner for manga). Factory constructors `MediaDetailsSheet.movie()` / `.tvShow()` eliminate 12-field mapping duplication across 3 call sites. `_RecPosterCard` and `_DiscoverPosterCard` replaced with unified `MediaPosterCard` — consistent hover effects, rating badges, and "in collection" indicators across search, discover, and recommendations
...
- **Backdrop in item detail card** — movies and TV shows display TMDB backdrop image as background in the detail card (gradient fade, 40% screen height). Visible through content with diagonal + vertical transparency
...
- **Tag sidebar** — vertical bookmark-style panel on the right side of collection view (desktop only). Appears when collection has 2+ tags. Multi-select: click tags to toggle, "All" resets. Filters items by selected tags across both grid and table views (`tag_sidebar.dart`, `collection_screen.dart`)
