...
### Changed
- **Test suite: prune visual-overfit assertions** — removed ~1000 assertions across ~15 test files that pinned tests to specific colours, icon constants, font sizes, paddings, and structural wrapper widgets (`Container` / `SizedBox` / `Padding`). Every such assertion would have broken on a cosmetic redesign without any real behavioural change. Kept what verifies behaviour (data flowing to UI, callbacks firing, conditional show/hide on state change, prop pass-through, collaborator calls); dropped what only pinned visuals. Deleted `app_colors_test.dart`, `app_typography_test.dart`, `app_theme_test.dart` entirely (pure design-constant tautologies). Rewrote `media_poster_card_test.dart`, `shimmer_loading_test.dart`, `star_rating_bar_test.dart`, `item_status_test.dart`, `step_indicator_test.dart`, `welcome_step_intro_test.dart`, `welcome_step_how_it_works_test.dart`, `snackbar_extension_test.dart` around behaviour. Trimmed chrome-visibility probes from `vgmaps_panel_test.dart`, `steamgriddb_panel_test.dart`, `canvas_image_item_test.dart`, `canvas_text_item_test.dart`, `discover_row_test.dart`, `dual_rating_badge_test.dart`, `screen_app_bar_test.dart`, `tier_row_test.dart`. ~190 tests removed or collapsed; 4617 tests still green, analyze clean
