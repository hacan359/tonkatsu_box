...
| `lib/core/database/dao/tag_dao.dart` | **DAO тегов коллекций**. CRUD для `collection_tags` + `setItemTag()` на `collection_items.tag_id`. Методы: `getTagsByCollection()`, `getAll()`, `createTag()`, `renameTag()`, `updateTagColor()`, `deleteTag()`, `setItemTag()`, `clearTagFromItems()`, `upsertAll()` |
