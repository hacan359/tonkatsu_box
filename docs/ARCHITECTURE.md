...
| `lib/core/api/tmdb_api.dart` | **TMDB API клиент**. Bearer token авторизация. Методы: `searchMovies(query, {year})`, `searchTvShows(query, {firstAirDateYear})`, `multiSearch()`, `getMovieDetails()`, `getTvShowDetails()`, `getPopularMovies()`, `getPopularTvShows()`, `getMovieGenres()`, `getTvGenres()`, `getSeasonEpisodes(tmdbShowId, seasonNumber)`, `setLanguage(language)`, `getMovieRecommendations()`, `getTvShowRecommendations()`, `getMovieReviews()`, `getTvShowReviews()`, `discoverMovies()`, `discoverTvShows()`, `findByImdbId()`, `findByTvdbId()`. Класс `TmdbFindResult` (movies + tvShows). Lazy-cached genre map (`_movieGenreMap`, `_tvGenreMap`) — resolves `genre_ids` to `genres` in all list endpoints. Cache cleared on `setLanguage()` and `clearApiKey()` |
| `lib/core/api/kodi_api.dart` | **Kodi JSON-RPC API клиент**. HTTP с Basic Auth. Методы: `ping()`, `getApplicationProperties()`, `getMovies()`, `getTvShows()`, `getEpisodes()`, `rawCall()`. Реконфигурация через `setConnection()`/`clearConnection()`. Request log (50 entries max) для debug-панели. `KodiApiException` с statusCode + detail. Провайдер: `kodiApiProvider` |
...
| `lib/core/database/dao/tag_dao.dart` | **DAO тегов коллекций**. CRUD для `collection_tags` + `setItemTag()` на `collection_items.tag_id`. Методы: `getTagsByCollection()`, `getTagById()`, `getAll()`, `createTag()`, `renameTag()`, `updateTagColor()`, `deleteTag()`, `setItemTag()`, `clearTagFromItems()`, `upsertAll()`, `findTagByNameCaseInsensitive()` (Unicode-safe поиск через Dart `toLowerCase`), `resolveOrCreateInCollection()` (get-or-create по имени — используется при move/clone элементов для переноса тега между коллекциями) |
...
| `lib/core/services/collection_hero_service.dart` | **Сервис hero-изображений коллекций**. Хранение в `<appSupport>/collections/hero_<id>_<ts>.<ext>`. Методы: `resolveRoot()` (инициализация каталога), `resolve()` (filename → абсолютный путь), `pickAndSave()` (file picker + копия + удаление старого), `saveBytes()` (для импорта из `.xcollx`), `delete()`, `listAll()` (для бэкапа). Провайдеры: `collectionsHeroDirProvider`, `collectionHeroServiceProvider` |
...
| `lib/core/services/discord_rpc_service.dart` | **Discord Rich Presence**. Показывает текущий элемент коллекции в статусе Discord через IPC pipe (desktop only). Методы: `enable()`, `disable()`, `updatePresence(item, {raData})`, `clearPresence()`. Дедупликация по ключу элемента, lazy reconnect. Activity verbs: Playing/Watching/Reading. RA-связанные игры показывают иконку + прогресс достижений. Провайдер: `discordRpcServiceProvider` |
...
| `lib/core/services/steam_import_service.dart` | **Сервис импорта Steam библиотеки**. Модели: `SteamImportStage` (enum), `SteamImportProgress` (stage/current/total/stats), `SteamImportResult` (imported/wishlisted/skipped/total/collectionId). Метод `importLibrary()`: fetch Steam library → filter DLC → batch lookup via `IgdbApi.lookupSteamGames()` (exact match by Steam appId, 2 HTTP requests) → for each game: add to collection (PC platform, status by playtime, `lastActivityAt` from `rtime_last_played`) or wishlist. Lazy collection creation via `createCollection` callback — collection only created after successful Steam library fetch. Провайдер: `steamImportServiceProvider` (зависит от steamApi, igdbApi, databaseService) |
| `lib/core/services/ra_import_service.dart` | **Сервис импорта RetroAchievements**. Модели: `RaImportStage` (enum: fetchingLibrary/matchingGames/completed), `RaImportProgress` (stage/current/total/currentName/addedCount/updatedCount/unmatchedCount), `RaImportResult` (totalGames/added/updated/unmatched/unmatchedTitles/collectionId) + `toUniversal()` extension. Метод `importFromProfile()`: fetch RA library → batch IGDB search via `IgdbApi.multiSearchGamesByName()` (10 games per multiquery, `RaToIgdbMapper.bestMatch()` for name matching) → add to collection (platform mapping, status by achievement progress) или update existing (status upgrade only, `lastActivityAt`/`completedAt` dates) → saves `tracker_game_data` per game for RA section → wishlist fallback для unmatched. Lazy collection creation via `createCollection` callback. `HighestAwardDate` parsed directly from RA response (no separate API call). Провайдер: `raImportServiceProvider` (зависит от raApi, igdbApi, databaseService, trackerDao) |
| `lib/core/services/tracker_sync_service.dart` | **Сервис синхронизации трекеров**. Quick sync RA: fetch completion progress → compare with `tracker_game_data` → skip unchanged, upsert changed. Lazy per-game: `getOrLoadRaAchievements()` loads from cache or API (`GetGameInfoAndUserProgress`). Profile sync: `syncRaProfile()`. Модели: `TrackerSyncProgress`, `TrackerSyncResult`. Провайдер: `trackerSyncServiceProvider` |
| `lib/core/database/dao/tracker_dao.dart` | **DAO для трекеров**. CRUD для `tracker_profiles`, `tracker_game_data`, `tracker_achievements`. Batch upsert, replace achievements (delete+insert в транзакции). Провайдер: `trackerDaoProvider` |
| `lib/core/services/ra_to_igdb_mapper.dart` | **Маппер RA → IGDB**. `RaToIgdbMapper` — поиск IGDB игры по RA данным. `consolePlatformMap` — `Map<int, List<int>>`: RA ConsoleID → список IGDB PlatformID (первый = основной, остальные = алиасы: Super Famicom, Family Computer, Neo Geo Pocket Color и др.). 56 RA консолей замаплены. `primaryIgdbPlatformId()` — основной IGDB ID для forward lookup. `igdbToRaConsoleIds()` — обратный маппинг (проверяет все алиасы). `findIgdbGame()` — поштучный поиск с platform filter → fallback без платформы. `bestMatch()` — public static: exact match → starts-with → first result. `normalize()` — lowercase, только буквы и цифры, cached `RegExp` |
| `lib/core/services/collection_browser_service.dart` | **Сервис каталога коллекций**. Загрузка `index.json` из GitHub-репозитория `tonkatsu-collections`, скачивание и парсинг `.xcoll`/`.xcollx`/`.zip` файлов. In-memory кэш индекса. Zip-извлечение через `archive` package. Класс `CollectionBrowserException`. Провайдер: `collectionBrowserServiceProvider` |
| `lib/core/services/kodi_sync_service.dart` | **Сервис синхронизации с Kodi**. Периодический опрос VideoLibrary.GetMovies → матчинг с TMDB → обновление/добавление в коллекцию. Поддержка sub-collections из Kodi movie sets. No-op guard: comment/rating обновляются только при изменении. Модели: `KodiSyncResult` (fetched/updated/added/errors/collectionsCreated). Методы: `start()`, `stop()`, `syncNow()`. Провайдер: `kodiSyncServiceProvider` |
...
| `lib/shared/models/item_status_logic.dart` | **Чистые функции логики статусов/дат**. `computeDatesForStatus()` — даты при смене статуса (startedAt/completedAt/clear). `computeStatusForDates()` — статус при установке дат через DatePicker. `computeStatusFromProgress()` — статус по прогрессу эпизодов/глав. `mergeExternalStatus()` — слияние локального и внешнего статуса (защита dropped, приоритет, allowDowngrade). Класс `StatusDatesUpdate`. Без Flutter-зависимостей |
| `lib/shared/models/kodi_movie.dart` | **Модель фильма Kodi**. DTO из VideoLibrary.GetMovies. Поля: movieId, title, year, playcount, lastPlayed, userRating, uniqueIds, set, dateAdded, communityRating. `fromJson()` с парсингом Kodi-специфичных форматов |
| `lib/shared/models/kodi_tv_show.dart` | **Модель сериала Kodi**. DTO из VideoLibrary.GetTVShows. Поля: tvShowId, title, year, playcount, lastPlayed, userRating, uniqueIds. `fromJson()` |
| `lib/shared/models/kodi_episode.dart` | **Модель эпизода Kodi**. DTO из VideoLibrary.GetEpisodes. Поля: episodeId, showTitle, season, episode, playcount, lastPlayed, uniqueIds. `fromJson()` |
| `lib/shared/models/kodi_unique_ids.dart` | **Внешние ID из Kodi** (TMDB/IMDB/TVDB). Парсит строки в числовые ID. Приоритет матчинга: tmdbId → imdbId → tvdbId |
| `lib/shared/models/kodi_application_info.dart` | **Информация о Kodi инстансе**. Версия + имя из Application.GetProperties |
| `lib/shared/models/kodi_date_parser.dart` | **Парсер дат Kodi**. `parseKodiDateTime()` — конвертирует `"YYYY-MM-DD HH:MM:SS"` → DateTime |
...
| `lib/shared/models/tracker_profile.dart` | **Профиль внешнего трекера** (RA, Steam, Trakt). Enum `TrackerType`. Поля: trackerType, userId, displayName, avatarUrl, profileUrl, totalPoints, totalGames, totalAchievements, profileData (JSON). Методы: `fromDb()`, `toDb()`, `copyWith()` |
| `lib/shared/models/tracker_game_data.dart` | **Прогресс per-game от трекера**. Привязан к `games.id` (IGDB). Поля: trackerType, gameId, trackerGameId, achievementsEarned/Total/Hardcore, awardKind, awardDate, playtimeMinutes, lastPlayedAt. Computed: `completionRate`, `hardcoreCompletionRate`, `isMastered`, `isBeaten`, `raGameUrl`. Методы: `fromDb()`, `toDb()`, `copyWith()` |
| `lib/shared/models/tracker_achievement.dart` | **Конкретное достижение per-game**. Поля: trackerType, trackerGameId, achievementId, title, description, points, badgeName, type (missable/progression), earned, earnedAt. Computed: `badgeUrl`, `lockedBadgeUrl`, `isMissable`. Методы: `fromDb()`, `toDb()`, `fromRaJson()`, `copyWith()` |
...
| `lib/features/collections/providers/rich_collections_provider.dart` | **`richCollectionsEnabledProvider`**. Лёгкий Provider<bool> для признака rich-вида коллекций — виджеты (`CollectionCard`, `CollectionListTile`) читают настройку без обязательного инстанцирования `SettingsNotifier`. Фолбэчится в `false`, если `settingsNotifierProvider` не инициализирован (unit-тесты) |
...
| `lib/features/collections/widgets/collection_card.dart` | **Диспетчер карточки коллекции**. Выбирает вариант отображения в зависимости от `richCollectionsEnabledProvider` и наличия `heroImagePath`: `ClassicCollectionCard` (мозаика 3+3) или `RichCollectionCard` (hero на всю карточку). Публичный API: `CollectionCard`, `UncategorizedCard` |
| `lib/features/collections/widgets/collection_card_shell.dart` | **Общий контейнер карточки коллекции**. Фокус-рамка, hover-анимация (AnimationController), InkWell, скругление. Используется и classic, и rich вариантами через builder-callback — различия только в контенте поверх dim-анимации |
| `lib/features/collections/widgets/collection_card_overlay.dart` | **Text-overlay карточки**. `CollectionCardOverlay` — имя → описание → статистика с responsive метриками (xs/sm/default в зависимости от ширины карточки). `CollectionCardBottomScrim` — нижний scrim для читаемости overlay'я поверх мозаики |
| `lib/features/collections/widgets/classic/classic_collection_card.dart` | **Classic-карточка**. Мозаика 3+3 обложек (3 сверху, 2 + "+N" снизу) поверх `CollectionCardShell`, общий `CollectionCardOverlay` поверх `CollectionCardBottomScrim` |
| `lib/features/collections/widgets/rich/rich_collection_card.dart` | **Rich-карточка**. `CollectionHeroBackground` (hero на всю площадь) + общий `CollectionCardOverlay` поверх `CollectionCardShell` |
| `lib/features/collections/widgets/rich/rich_collection_body.dart` | **Hero-баннер на экране коллекции** (`RichHeroBanner`). Картинка справа, имя/описание слева, мягкий fade снизу. Используется как sliver-header в `CollectionItemsView` — скроллится вместе с гридом. Фолбэк на дефолтную картинку через `defaultHeroAssetForCollection` |
| `lib/features/collections/widgets/rich/default_hero_assets.dart` | **Фолбэк-картинки для rich-коллекций без своего hero**. Список имён файлов в `assets/images/collection_hero_defaults/`, детерминированный выбор по `id % N` |
| `lib/features/collections/widgets/collection_hero_background.dart` | **Фоновая подложка с hero**. Картинка с `BoxFit.cover, alignment: topRight` + два градиента (слева→прозрачный, снизу→прозрачный), оба заканчиваются в `AppColors.background`. Параметризация через `HeroGradientStrength` (`soft` для карточек / `standard` для баннера), cache width × DPR |
| `lib/features/collections/widgets/edit_collection_dialog.dart` | **Диалог редактирования коллекции**. Имя, описание, обложка. Preview через `CollectionHeroBackground`, пикер/удаление через `CollectionHeroService`. Сохраняет напрямую через `collectionsProvider.notifier` |
...
| `lib/features/collections/widgets/collection_filter_bar.dart` | **Панель фильтров** (`ConsumerStatefulWidget`). Строка type chips (multi-select `ChoiceChip`), поиск, сортировка. Desktop: collapsible panel с platform чипсами (`AnimatedCrossFade`), tag chips managed by TagSidebar. Mobile: bottom sheet с `ChoiceChip` группами + "Group" toggle chip for tag grouping. Clear button для сброса всех фильтров |
...
| `lib/features/collections/widgets/collection_table_view.dart` | **Табличный вид коллекции**. 8 sortable columns (Name, Type, Platform, Status, Tag, Rating, Year, Added), cyclic header filters (Status/Type/Rating/Tag/Platform), inline-edit popups for Rating (1–10 stars), Status (5 options), Tag (assign/remove). Drag-and-drop reorder via `ReorderableListView` when `onReorder` is provided (manual sort mode) — column-click sort/filter disabled in that mode. Sticky header (UPPERCASE labels), zebra row striping, poster thumbnails (36×52), media type icons, status chips with colored dot, tag chips, star ratings, hover highlight |
| `lib/features/collections/widgets/tag_sidebar.dart` | **Боковая панель тегов**. Vertical bookmark tabs on the right side for multi-select tag filtering. Appears when 1+ tags exist. "Group" button toggles tag grouping mode. Color-coded active state with left border accent |
...
| `lib/features/collections/widgets/recommendations_section.dart` | **Секция рекомендаций**. ConsumerWidget. Горизонтальные ряды "Similar Movies" и "Similar TV Shows" из TMDB `/similar`. `_RecommendationRow` с `ScrollableRowWithArrows`. Tap → `ItemDetailsSheet` с кнопкой "Add to Collection". Параметры: `onAddMovie`, `onAddTvShow` |
...
| Файл | Назначение |
|------|------------|
| `lib/features/collections/models/collections_index.dart` | **Модели каталога онлайн-коллекций**. 5 классов для парсинга `index.json`: `RemotePlatform` (id, name, shortName, igdbId, manufacturer, releaseYear), `RemoteMediaType` (id, name, source), `CollectionCategory` (id, name, description), `RemoteCollection` (id, name, mediaType, platform, category, itemsCount, author, format, file, size + `sizeFormatted` getter), `CollectionsIndex` (version, totalCollections, totalItems, platforms[], mediaTypes[], collections[], categories[]). Все с `fromJson` factory |
| `lib/features/collections/providers/collection_browser_provider.dart` | **Провайдеры каталога коллекций**. `collectionsIndexProvider` (AsyncNotifierProvider — загрузка index.json, refresh), `browserPlatformFilterProvider` / `browserCategoryFilterProvider` (StateProvider\<String?\>), `browserSearchQueryProvider` (StateProvider\<String\>), `filteredRemoteCollectionsProvider` (Provider — фильтрация по платформе/категории/поиску) |
...
| Файл | Назначение |
|------|------------|
| `lib/features/wishlist/screens/wishlist_screen.dart` | **Экран вишлиста**. ListView с `_WishlistTile`, FAB для добавления, фильтр resolved (visibility toggle), clear resolved с confirmation. Context menu (right-click / long press): Search/Edit/Resolve/Delete via `showMenu`. Тап на элемент → `SearchScreen(initialQuery)`. Resolved: opacity 0.5, strikethrough |
...
| `lib/features/search/widgets/item_details_sheet.dart` | **Универсальный bottom sheet деталей**. Единый `ItemDetailsSheet` с factory конструкторами (`.movie()`, `.tvShow()`, `.game()`, `.manga()`, `.visualNovel()`). Rounded Material с elevation, tiled background, full-bleed backdrop с gradient fade, translucent content card, circular add button с hover. Модульные параметры: `subtitle`, `infoChips`, `extraInfoIcon`, `maxGenres`, `coverHeight` |
...
| `lib/shared/widgets/screen_app_bar.dart` | **Единый AppBar**. Compact 44px, gradient border (surface → background), localized title (14px semibold), automatic back button on mobile via `canPop()`. `_CopyableTitle`: click copies title to clipboard, hover shows copy icon → checkmark on success. Implements `PreferredSizeWidget`. Parameters: `title`, `actions`, `bottom` (TabBar). Used by all screens |
...
| `lib/features/settings/screens/kodi_screen.dart` | **Единый экран Kodi**: подключение (host/port/username/password), test connection, sync controls (enable/interval/sub-collections/ratings), debug panel (sync status, request log, raw JSON-RPC console). ConsumerStatefulWidget |
| `lib/features/settings/providers/kodi_settings_provider.dart` | **State настроек Kodi**. Per-profile persistence через SharedPreferences. `KodiSettingsKeys` — ключи с profileId суффиксом. `KodiSettingsState` — 11 полей (enabled, host, port, username, password, syncInterval, importRatings, targetCollectionId, createSubCollections, lastSyncTimestamp, addUnmatchedToWishlist). Auto-start sync в `build()` при enabled + hasConnection + targetCollectionId. Автообновление `KodiApi.setConnection()` при смене host/port/creds. Провайдер: `kodiSettingsProvider` |
