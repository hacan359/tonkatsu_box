...
| `lib/features/collections/widgets/collection_filter_bar.dart` | **Панель фильтров** (`ConsumerStatefulWidget`). Строка type chips (multi-select `ChoiceChip`), поиск, сортировка. Desktop: collapsible panel с platform/tag чипсами (`AnimatedCrossFade`). Mobile: bottom sheet с `ChoiceChip` группами. Clear button для сброса всех фильтров |
