...
| `lib/core/services/ra_to_igdb_mapper.dart` | **Маппер RA → IGDB**. `RaToIgdbMapper` — поиск IGDB игры по RA данным. `consolePlatformMap` — статическая таблица 30+ RA ConsoleID → IGDB PlatformID (Genesis, N64, SNES, GB, GBA, GBC, NES, PS1, PS2, PSP, Dreamcast, DS, 3DS, Arcade и др.). `findIgdbGame()` — поштучный поиск с platform filter → fallback без платформы (используется в fallback path). `bestMatch()` — public static: exact match → starts-with → first result. `normalize()` — lowercase, только буквы и цифры, cached `RegExp` |
