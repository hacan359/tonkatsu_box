...
> ⚠️ **На Windows поддержка геймпада временно отключена.** Нативный плагин
> `gamepads_windows` падает с access violation (`0xc0000005`) в потоке опроса
> устройств у части пользователей, поэтому опрос на Windows не запускается
> (см. `kGamepadSupported`). Геймпад работает на Linux, macOS и Android.

Приложение поддерживает полное управление Xbox-контроллером.
...
- `MediaPosterCard`, `SelectablePosterCard` — Material > InkWell
- `CollectionItemTile`, `CollectionListTile` — Material > InkWell
...
```
test/core/services/gamepad_service_test.dart               — 32 теста (нормализация, POV, триггеры, debounce)
test/shared/gamepad/gamepad_provider_test.dart              — 13 тестов (InputMode, enum values)
