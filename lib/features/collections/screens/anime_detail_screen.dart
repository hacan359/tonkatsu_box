// Экран детального просмотра анимации в коллекции.
...
/// Экран детального просмотра анимации в коллекции.
///
/// Адаптивный: показывает movie-like layout (runtime, без episode tracker)
/// для [AnimationSource.movie] и tvShow-like layout (episode tracker, seasons)
/// для [AnimationSource.tvShow]. Определяется по [CollectionItem.platformId].
class AnimeDetailScreen extends ConsumerStatefulWidget {
  /// Создаёт [AnimeDetailScreen].
...
class _AnimeDetailScreenState extends ConsumerState<AnimeDetailScreen>
  ...
  /// Является ли элемент анимационным сериалом.
  bool _isTvShowSource(CollectionItem item) {
  ...
  Widget _buildMovieDetails(CollectionItem item) {
  ...
  List<MediaDetailChip> _buildMovieInfoChips(Movie? movie) {
  ...
  Widget _buildTvShowDetails(CollectionItem item) {
  ...
  Future<void> _updateStatus(int id, ItemStatus status) async {
    await ref
        ...
        .updateStatus(id, status, MediaType.animation);
  }
...
/// Виджет списка сезонов для анимационного сериала.
class _AnimeSeasonsListWidget extends ConsumerStatefulWidget {
  const _AnimeSeasonsListWidget({
  ...
  @override
  ConsumerState<_AnimeSeasonsListWidget> createState() =>
      _AnimeSeasonsListWidgetState();
}

class _AnimeSeasonsListWidgetState
    extends ConsumerState<_AnimeSeasonsListWidget> {
...
/// ExpansionTile для одного сезона анимационного сериала.
class _AnimeSeasonExpansionTile extends ConsumerWidget {
  const _AnimeSeasonExpansionTile({
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return ExpansionTile(
      ...
      leading: Icon(
        ...
        color: allWatched ? AppColors.animationAccent : AppColors.surfaceBorder,
        size: 20,
      ),
      title: Text(
        seasonTitle,
      ...
      children: <Widget>[
        if (isLoading)
          ...
          ...episodes.map((TvEpisode episode) => _AnimeEpisodeTile(
                episode: episode,
...
/// Тайл одного эпизода анимационного сериала.
class _AnimeEpisodeTile extends ConsumerWidget {
  const _AnimeEpisodeTile({
